...
import { CreditCard, DollarSign, CheckCircle, Clock, Search, Edit2, X, Save } from 'lucide-react';
...
export default function PaymentTracker() {
    ...
    const handleEditClick = (p) => {
        setEditingPayment(p);
        setEditValues({ PAYMENTSTATUS: p.PAYMENTSTATUS, PAYMENTMETHOD: p.PAYMENTMETHOD || 'Not Set', TRANSACTIONID: p.TRANSACTIONID || '' });
    };
    ...
    const handleUpdate = async () => {
        ...
        try {
            const { error } = await supabase
                .from('payment')
                .update({ paymentstatus: editValues.PAYMENTSTATUS, paymentmethod: editValues.PAYMENTMETHOD, transactionid: editValues.TRANSACTIONID })
    ...
    const filtered = payments.filter(p => {
        ...
        const matchesSearch = !q || String(p.PAYMENTID).includes(q) || String(p.SHIPMENTID).includes(q) || (p.CUSTOMER_NAME || '').toLowerCase().includes(q) || (p.PAYMENTMETHOD || '').toLowerCase().includes(q);
        ...
        <div style={{ position: 'relative' }}>
                    ...
                    {statCards.map(card => (
                        <div key={card.label} className="card stat-card" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '8px', transition: 'all 0.3s ease' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                <div className="icon-wrapper" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '56px', height: '56px', borderRadius: '50%', background: `linear-gradient(135deg, ${card.color}20, ${card.color}05)`, color: card.color, boxShadow: `0 4px 16px ${card.color}30`, transition: 'transform 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)' }}>
                                    {card.icon}
                                </div>
                                <div style={{ width: '10px', height: '10px', borderRadius: '50%', background: card.color, boxShadow: `0 0 8px ${card.color}80` }}></div>
                            </div>
                            <div style={{ fontSize: '1.7rem', fontWeight: '800', color: card.color, letterSpacing: '-1px', marginTop: '8px' }}>{card.value}</div>
            ...
            <div style={{ display: 'flex', gap: '12px', marginBottom: '20px', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                    {[{ id: 'All', label: 'All Payments' }, { id: 'Pending', label: 'Pending' }, { id: 'Paid', label: 'Completed' }].map(f => (
            ...
            <div className="card" style={{ padding: '24px', overflowX: 'auto' }}>
                {loading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '64px', color: 'var(--text-muted)' }}>
                        <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '16px', opacity: 0.3 }}><CreditCard size={48} /></div>
                            ...
                            {filtered.map((p, i) => {
                                ...
                                return (
                                    <tr key={p.PAYMENTID} style={{ background: 'var(--bg-body)' }}>
                                        <td style={{ padding: '16px', fontWeight: '700', color: 'var(--accent-primary)', borderRadius: '12px 0 0 12px', borderTop: '1px solid var(--border-color)', borderBottom: i === filtered.length - 1 ? '1px solid var(--border-color)' : 'none' }}>#{p.PAYMENTID}</td>
                                        <td style={{ padding: '16px', color: 'var(--text-muted)', borderTop: '1px solid var(--border-color)', borderBottom: i === filtered.length - 1 ? '1px solid var(--border-color)' : 'none' }}>#{p.SHIPMENTID}</td>
                                        <td style={{ padding: '16px', color: 'var(--text-main)', fontWeight: '600', borderTop: '1px solid var(--border-color)', borderBottom: i === filtered.length - 1 ? '1px solid var(--border-color)' : 'none' }}>{p.CUSTOMER_NAME}</td>
                                        <td style={{ padding: '16px', fontWeight: '800', fontSize: '1.05rem', color: 'var(--text-main)', borderTop: '1px solid var(--border-color)', borderBottom: i === filtered.length - 1 ? '1px solid var(--border-color)' : 'none' }}>{fmt(p.AMOUNT)}</td>
                                        <td style={{ padding: '16px', color: 'var(--text-muted)', borderTop: '1px solid var(--border-color)', borderBottom: i === filtered.length - 1 ? '1px solid var(--border-color)' : 'none' }}>{p.PAYMENTMETHOD || '—'}</td>
                                        <td style={{ padding: '16px', color: 'var(--text-muted)', borderTop: '1px solid var(--border-color)', borderBottom: i === filtered.length - 1 ? '1px solid var(--border-color)' : 'none' }}>{p.PAYMENTDATE ? new Date(p.PAYMENTDATE).toLocaleDateString() : '—'}</td>
                                        <td style={{ padding: '16px', borderTop: '1px solid var(--border-color)', borderBottom: i === filtered.length - 1 ? '1px solid var(--border-color)' : 'none' }}>
                                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '5px 14px', borderRadius: '99px', background: st.bg, color: st.color, fontWeight: '700', fontSize: '0.78rem', textTransform: 'uppercase' }}>
                                        ...
                                        <td style={{ padding: '16px', borderRadius: '0 12px 12px 0', borderTop: '1px solid var(--border-color)', borderBottom: i === filtered.length - 1 ? '1px solid var(--border-color)' : 'none' }}>
                                            <button onClick={() => handleEditClick(p)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: '6px', borderRadius: '8px', transition: 'all 0.2s' }}
            ...
            {editingPayment && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <div style={{ width: '400px', background: 'var(--bg-card)', padding: '32px', borderRadius: '24px', boxShadow: 'var(--shadow-lg)', border: '1px solid var(--border-color)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                            <h3 style={{ margin: 0, fontSize: '1.2rem', fontWeight: '700' }}>Update Payment #{editingPayment.PAYMENTID}</h3>
                            <button onClick={() => setEditingPayment(null)} style={{ background: 'transparent', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}><X size={20} /></button>
                        </div>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            <div>
                                ...
                                <select value={editValues.PAYMENTSTATUS} onChange={(e) => setEditValues({...editValues, PAYMENTSTATUS: e.target.value})} style={{ width: '100%', padding: '12px', borderRadius: '12px', background: 'var(--bg-body)', border: '1px solid var(--border-color)', color: 'var(--text-main)', fontSize: '0.95rem', outline: 'none' }}>
                            ...
                            <div>
                                ...
                                <select value={editValues.PAYMENTMETHOD} onChange={(e) => setEditValues({...editValues, PAYMENTMETHOD: e.target.value})} style={{ width: '100%', padding: '12px', borderRadius: '12px', background: 'var(--bg-body)', border: '1px solid var(--border-color)', color: 'var(--text-main)', fontSize: '0.95rem', outline: 'none' }}>
                            ...
                            <div>
                                ...
                                <input type="text" value={editValues.TRANSACTIONID} onChange={(e) => setEditValues({...editValues, TRANSACTIONID: e.target.value})} placeholder="Enter or generate ID"
                                    style={{ width: '100%', padding: '12px', borderRadius: '12px', background: 'var(--bg-body)', border: '1px solid var(--border-color)', color: 'var(--text-main)', fontSize: '0.95rem', outline: 'none' }} />
                            </div>
                            <button onClick={handleUpdate} disabled={isUpdating} style={{ marginTop: '12px', width: '100%', padding: '14px', borderRadius: '12px', background: 'var(--accent-primary)', color: 'white', border: 'none', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', opacity: isUpdating ? 0.7 : 1 }}>
            ...
            <style>{`.stat-card:hover .icon-wrapper { transform: scale(1.05); }`}</style>
        </div>
    );
}